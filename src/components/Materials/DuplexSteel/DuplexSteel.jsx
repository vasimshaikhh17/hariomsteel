...
const DuplexSteel = () => {
  ...
  const duplexSteel = materialGrade.find(
    (item) => item.title === "Duplex Steel"
  );

  return (
    <Layout>
      <div className="py-8 customContainer">
        <h2 className="text-3xl font-bold text-center mb-8">Duplex Steel</h2>
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {duplexSteel.sub_sub_category.map((subCategory) => (
              <div
                key={subCategory._id}
                className="bg-white shadow-lg rounded-lg overflow-hidden"
              >
                <img
                  src={subCategory.prodImg}
